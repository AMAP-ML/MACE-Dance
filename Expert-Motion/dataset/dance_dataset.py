...
class AISTPPDataset(Dataset):
    def __init__(
        ...
        print(f"Loaded {self.name} Dataset With Dimensions: Pos: {data['pos'].shape}, Q: {data['q'].shape}, features: {data['features'].shape}")
    ...
    def __getitem__(self, idx):
        ...
        return self.data["pose"][idx], self.data['features'][idx], filename_, self.data["wavs"][idx], self.data['genres'][idx]
    ...
    def load_aistpp(self):
        ...
        genre_path = os.path.join(split_data_path, f"label")
        ...
        for motion, name, wav in zip(motions, features, wavs):
            # make sure name is matching
            m_name = os.path.splitext(os.path.basename(motion))[0]
            f_name = os.path.splitext(os.path.basename(name))[0]
            w_name = os.path.splitext(os.path.basename(wav))[0]
            g_name = os.path.splitext(os.path.basename(wav))[0][:3] + '.json'
            assert m_name == f_name == w_name, str((motion, name, wav))
            # load motion
            data = pickle.load(open(motion, "rb"))
            pos = data["pos"]
            q = data["q"]
            feature = np.load(name)
            with open(os.path.join(genre_path, g_name), 'r') as f:
                genre = style2s.index(json.load(f).get('style2'))
            all_pos.append(pos)
            all_q.append(q)
        ...
        all_features = np.array(all_features) # N x seq x 35
        all_genres = np.array(all_genres) # N * 1

        # downsample the motions to the data fps
        all_pos = all_pos[:, ::self.data_stride, :]
        all_q = all_q[:, ::self.data_stride, :]
        all_features = all_features[:, ::self.data_stride, :]
        data = {"pos": all_pos, "q": all_q, "filenames": all_names, "wavs": all_wavs, 'features': all_features, 'genres': all_genres}
        return data
    ...
    def process_dataset(self, root_pos, local_q):
        ...
        h1 = global_pose_vec_input.clone()
